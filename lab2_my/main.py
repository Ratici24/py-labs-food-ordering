...
from database import Base, engine, get_db, raw_conn
import models
import schemas
...
app = FastAPI(
    ...
    openapi_tags=[
        ...
        {"name": "Stats", "description": "Аналітика на сирому SQL (psycopg)"},
    ],
)
...
def menu_page(request: Request, q: str = "", db: Session = Depends(get_db)):
    dish_q = db.query(models.Dish)
    if q:
        dish_q = dish_q.filter(models.Dish.name.ilike(f"%{q}%"))
    dishes = dish_q.all()
    categories = db.query(models.Category).all()
    return templates.TemplateResponse(
        request, "menu.html",
        {"categories": categories, "dishes": dishes, "q": q},
    )
...
def stats_page(request: Request):
    sql = """
        SELECT d.name, COALESCE(SUM(o.quantity), 0) AS total
        FROM dishes d
